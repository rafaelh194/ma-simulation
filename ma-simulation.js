    ...
    const archetypeDefaults = {
      'Single Owner': {
        revenue_comp: { personal: [30, 60], corp: [10, 20], rec: [10, 40], adhoc: [5, 15] },
        revenue: { min: 800, ml: 1000, max: 1400 },
        ebitda: { min: 35, ml: 40, max: 45 },
        multiple: { avg: 3, std: 0.75 },
        synergies: { cross: [5, 0.5], price: [5, 0.5], churn: [5, 0.5], tech: [5, 0.5], shared: [5, 0.5], facility: [5, 0.5] },
        rollover: { min: 0, ml: 10, max: 20 },
        earnout: { min: 0, ml: 5, max: 10, years: 2 },
        sellerFin: { min: 0, ml: 0, max: 10, rate: 5, term: 60 },
        extra_exp: 0,
        timing: { year: 2025, month: 1 }
      },


      'Small Firm': {
        revenue_comp: { personal: [30, 60], corp: [10, 20], rec: [10, 40], adhoc: [5, 15] },
        revenue: { min: 1400, ml: 2000, max: 2600 },
        ebitda: { min: 30, ml: 35, max: 40 },
        multiple: { avg: 4, std: 0.75 },
        synergies: { cross: [5, 0.5], price: [5, 0.5], churn: [5, 0.5], tech: [5, 0.5], shared: [5, 0.5], facility: [5, 0.5] },
        rollover: { min: 0, ml: 10, max: 20 },
        earnout: { min: 0, ml: 5, max: 10, years: 2 },
        sellerFin: { min: 0, ml: 0, max: 10, rate: 5, term: 60 },
      ...
      'Mid Firm': {
        revenue_comp: { personal: [30, 60], corp: [10, 20], rec: [10, 40], adhoc: [5, 15] },
        revenue: { min: 3000, ml: 4000, max: 5000 },
        ebitda: { min: 28, ml: 33, max: 38 },
        multiple: { avg: 5, std: 0.75 },
        synergies: { cross: [5, 0.5], price: [5, 0.5], churn: [5, 0.5], tech: [5, 0.5], shared: [5, 0.5], facility: [5, 0.5] },
        rollover: { min: 0, ml: 10, max: 20 },
        earnout: { min: 0, ml: 5, max: 10, years: 2 },
        sellerFin: { min: 0, ml: 0, max: 10, rate: 5, term: 60 },
    ...
    function renderCharts(
        ...
        Plotly.newPlot("ebitdaBandChart", [
            { x: yearLabels, y: p90, name: "P90", type: "scatter", line: { color: "rgba(75, 192, 192, 1)" }, fill: "tonexty" },
            { x: yearLabels, y: p50, name: "P50", type: "scatter", line: { color: "rgba(54, 162, 235, 1)" }, fill: "tonexty" },
            { x: yearLabels, y: p10, name: "P10", type: "scatter", line: { color: "rgba(255, 99, 132, 1)" } }
        ], {
        ...
        Plotly.newPlot("finalYearHistogram", [{
            ...
            line: { width: 1, color: "rgba(153, 102, 255, 1)" }
            }
        }], {
            title: "Final Year EBITDA Distribution",
            xaxis: { title: "EBITDA ($)", tickformat: ",", automargin: true },
            yaxis: { title: "Count" },
            bargap: 0.1,
        ...
        const cdf = sorted.map((v, i) => ({ x: v, y: (i + 1) / sorted.length }));
    ...
    function runSimulation() {
      ...
      const revenueTotals = Array.from({ length: years }, () => Array(months).fill(0));
      const costTotals = Array.from({ length: years }, () => Array(months).fill(0));
      const ebitdaTotals = Array.from({ length: years }, () => Array(months).fill(0));
      const revenueCount = Array.from({ length: years }, () => Array(months).fill(0));
      const costCount = Array.from({ length: years }, () => Array(months).fill(0));
      const ebitdaCount = Array.from({ length: years }, () => Array(months).fill(0));
      ...
      const debtScheduleByCompany = Array.from({ length: numCompanies }, () =>
        Array.from({ length: years }, () => Array(months).fill(0))
      );

      for (let run = 0; run < NUM_RUNS; run++) {
            const revenue = Array.from({ length: years }, () => Array(months).fill(0));
            const cost = Array.from({ length: years }, () => Array(months).fill(0));
            const ebitda = Array.from({ length: years }, () => Array(months).fill(0));
            ...
            for (let i = 0; i < numCompanies; i++) {
                ...
                let adhocWeights = Array.from({ length: months }, () => Math.random());
                ...
                for (let m = 0; m < months; m++) revMonthly[m] *= normFactor;
                    ...
                    for (let m = 0; m < sellerTerm; m++) {
                        ...
                        if (!sellerDebtScheduleByCompany[i].interest) sellerDebtScheduleByCompany[i].interest = Array.from({ length: years }, () => Array(months).fill(0));
                        if (!sellerDebtScheduleByCompany[i].principal) sellerDebtScheduleByCompany[i].principal = Array.from({ length: years }, () => Array(months).fill(0));
